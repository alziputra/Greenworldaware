      ...
      {loading ? (
        ...
        petitions.map((item) => (
          <Link to={`/petisi/${item.id}`} className="flex flex-col bg-white border border-gray-200 rounded-lg shadow w-full lg:h-40 md:h-full md:flex-row md:w-full md:border hover:bg-gray-100" key={item.id}>
            <img className="object-cover rounded-t-lg h-full md:w-48 md:rounded-none md:rounded-s-lg" src={item.image || "https://source.unsplash.com/random/600x700/?nature"} alt="Petition image" />
